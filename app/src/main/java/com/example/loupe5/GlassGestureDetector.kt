...
class GlassGestureDetector(context: Context, private val onGestureListener: OnGestureListener) :
    ...
    override fun onFling(
        ...
        return if (tan > TAN_60_DEGREES) {
            if (abs(deltaY) < SWIPE_DISTANCE_THRESHOLD_PX || abs(velocityY) < SWIPE_VELOCITY_THRESHOLD_PX) {
                false
            } else if (deltaY < 0) {
            ...
            if (abs(deltaX) < SWIPE_DISTANCE_THRESHOLD_PX || abs(velocityX) < SWIPE_VELOCITY_THRESHOLD_PX) {
                false
            } else if (deltaX < 0) {
